...
create policy if not exists "Trip members can view debt backups"
...
create policy if not exists "Trip owners can insert debt backups"
  on public.trip_debt_backups
  for insert
  with check (
    exists (
...
begin
  ...
  while exists(select 1 from tmp_creditors) and exists(select 1 from tmp_debtors) loop
    -- pick top creditor and debtor (largest amounts)
    -- order by balance desc
    perform 1;
    -- creditor
    declare v_c_name text; v_c_amt numeric; v_d_name text; v_d_amt numeric; v_settle numeric; begin end;
  end loop;

  -- The above is complex to express inline; implement with explicit queries
  loop
