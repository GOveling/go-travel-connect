...
-- First, create unique constraint on place_visits if it doesn't exist
ALTER TABLE place_visits 
ADD CONSTRAINT unique_user_place_visit 
UNIQUE (user_id, saved_place_id);

-- Now backup any visit data that exists in saved_places but not in place_visits
...
WHERE sp.visited = true
  AND NOT EXISTS (
    ...
    WHERE pv.saved_place_id = sp.id AND pv.user_id = t.user_id
  )
