...
const isValidInvitation = async (
  ...
  if (!invitation) {
    console.log("❌ No invitation found with token and email:", {
...
serve(async (req) => {
  ...
  try {
    ...
    const validationResult = await isValidInvitation(
      supabaseClient,
      token,
      user.email
    );
    ...
    console.log("📋 Invitation details:", {
      ...
      expired: invitationCheck
        ? new Date(invitationCheck.expires_at) < new Date()
        : "unknown",
      userEmail: user.email,
    ...
    if (!success) {
      console.log(
        "❌ Database function returned false - invitation invalid/expired"
      );
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.log(
      "✅ Invitation accepted successfully for trip:",
