...
import { MapPin, Users, Eye, Navigation, ZoomIn, ZoomOut, Layers, TrendingUp, Bookmark } from "lucide-react";
...
const TripMapInteractive = ({ trips }: TripMapInteractiveProps) => {
  ...
  const createCustomIcon = (status: string, emoji: string, type: 'destination' | 'savedPlace' = 'destination') => {
    const color = status === 'upcoming' ? '#10b981' : 
                 status === 'planning' ? '#8b5cf6' : '#6b7280';
        ...
        <CardContent className="p-0">
          <div className="h-96 lg:h-[500px] relative">
            ...
            >
              ...
              {filteredTrips.map((trip) =>
                trip.coordinates?.map((coord: any, index: number) => (
                  <Marker
                    ...
                    icon={createCustomIcon(trip.status, trip.image || '📍', 'destination')}
                  >
                    <Popup>
                      <div className="p-2 min-w-[200px]">
                        <h4 className="font-semibold text-lg mb-2">{trip.name}</h4>
                        <p className="text-sm text-gray-600 mb-2">{coord.name}</p>
                        ...
                        {trip.isGroupTrip && (
                          <div className="flex items-center space-x-1 bg-purple-100 px-2 py-1 rounded-full mt-2">
                            ...
                            <span className="text-xs text-purple-600">Viaje Grupal</span>
                          </div>
                        )}
                      </div>
                    </Popup>
              ...
              {showSavedPlaces && filteredTrips.map((trip) =>
                trip.savedPlaces?.filter((place: any) => place.lat && place.lng)
                  .map((place: any, index: number) => (
                    <Marker
                      key={`saved-${trip.id}-${place.id}`}
                      position={[place.lat, place.lng]}
                      icon={createCustomIcon(trip.status, place.image || '📍', 'savedPlace')}
                    >
                      <Popup>
                        <div className="p-2 min-w-[200px]">
                          <h4 className="font-semibold text-lg mb-2">{place.name}</h4>
                          <p className="text-sm text-gray-600 mb-2">{place.destinationName}</p>
                          <div className="flex items-center space-x-2 mb-2">
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500">
                            ...
                            <span className={`px-2 py-1 rounded ${
                              place.priority === 'high' ? 'bg-red-100 text-red-800' :
                              place.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' :
              ...
              {/* Trip Routes */}
              {showRoutes && filteredTrips.map((trip) => {
                ...
                return (
                  <Polyline
                    ...
                    weight={3}
                    opacity={0.7}
                    dashArray={trip.status === 'planning' ? '10, 10' : undefined}
                  />
                );
              })}
            </MapContainer>
      ...
      <Card>
        <CardContent className="p-4">
          ...
          <div className="space-y-3">
            <div>
              <h5 className="text-sm font-medium text-gray-700 mb-2">Estados de Viaje:</h5>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-700">En Planificación</span>
                </div>
                <div className="flex items-center space-x-2">
            ...
            {showRoutes && (
              <div className="pt-3 border-t">
                <h5 className="text-sm font-medium text-gray-700 mb-2">Rutas:</h5>
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <div className="w-6 h-0.5 bg-purple-600"></div>
                    <span className="text-sm text-gray-700">Ruta Confirmada</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-6 h-0.5 bg-purple-600" style={{ background: 'repeating-linear-gradient(to right, #8b5cf6, #8b5cf6 3px, transparent 3px, transparent 6px)' }}></div>
                    <span className="text-sm text-gray-700">Ruta en Planificación</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="pt-3 border-t">
              <div className="text-xs text-gray-500 space-y-1">
                ...
                <div>• Los lugares guardados aparecen como marcadores pequeños</div>
                <div>• Usa los filtros para personalizar la vista del mapa</div>
                <div>• Selecciona un viaje específico para ver solo sus destinos</div>
              </div>
            </div>
          </div>
        </CardContent>
