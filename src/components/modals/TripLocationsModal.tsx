...
export const TripLocationsModal = ({
  ...
  const fetchSharedLocations = async () => {
    try {
      ...
      // Add user profiles to locations using secure function
      const locationsWithProfiles = await Promise.all(
        (data || []).map(async (location) => {
          // Use the secure function to get collaborator profile data
          const { data: profileData } = await supabase
            .rpc("get_collaborator_profile_safe", { p_user_id: location.user_id });
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden rounded-lg">
        ...
        <div className="flex flex-col gap-4 h-[600px]">
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-blue-400 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white shadow-lg"
          >
