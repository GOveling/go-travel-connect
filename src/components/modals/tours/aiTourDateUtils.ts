...
export const getAITourBookingPlan = (trip: Trip): AITourBookingPlan => {
  ...
  destinationRanges.forEach((range, index) => {
    ...
    const savedPlacesForDestination =
    ...
    if (savedPlacesForDestination.length > 0) {
      ...
      const categories = savedPlacesForDestination.map((place) =>
      ...
      if (categories.includes("museum") || categories.includes("historical")) {
        ...
        reason =
          "IA detectó lugares históricos/culturales en lugares guardados";
      } else if (
      ...
      optimizationNotes.push(
        `${destination.name}: ${savedPlacesForDestination.length} lugares analizados para recomendaciones`
      );
    } else {
      ...
      reason =
        "Recomendación general - considera agregar lugares en AI Smart Route";
      optimizationNotes.push(
        `${destination.name}: Sin lugares guardados - recomendaciones generales`
      );
    }
  ...
  if (totalSavedPlaces >= 5) {
    confidence = "high";
    optimizationNotes.push(
      "Alta confianza - muchos lugares guardados para análisis"
    );
  } else if (totalSavedPlaces > 0) {
    confidence = "medium";
    optimizationNotes.push(
      "Confianza media - algunos lugares guardados disponibles"
    );
  } else {
    confidence = "low";
    optimizationNotes.push(
      "Baja confianza - sin lugares guardados para análisis específico"
    );
  }
...
export const formatTourBookingSummary = (plan: AITourBookingPlan): string => {
  const destinations = plan.recommendations
...
export const getTotalTourBudgetEstimate = (plan: AITourBookingPlan): string => {
  ...
  const estimatedTotal =
    totalTours * avgTourPrice * (plan.recommendations[0]?.participants || 2);
