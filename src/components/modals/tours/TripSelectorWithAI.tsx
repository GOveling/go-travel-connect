                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-500">
                    ...
                    {trip.savedPlaces && trip.savedPlaces.length > 0 && 
                      <span className="ml-1 text-orange-600">({trip.savedPlaces.length} lugares)</span>
                    }
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2 text-sm text-orange-800">
                <Camera size={14} />
                <span>Lugares guardados: {selectedTrip.savedPlaces?.length || 0}</span>
              </div>
                ...
                <div className="mt-3 p-2 bg-white rounded border border-orange-200">
                  ...
                  <ul className="text-xs text-orange-800 space-y-1">
                    {selectedTrip.coordinates?.map((dest, index) => {
                      const placesInDest = selectedTrip.savedPlaces?.filter(p => p.destinationName === dest.name).length || 0;
                      return (
                        <li key={index}>• {dest.name}: {placesInDest} lugares guardados</li>
                      );
                    })}
                  </ul>
                  <p className="text-xs text-orange-600 mt-2 font-medium">
                    ✨ Recomendaciones automáticas según lugares guardados
                  </p>
                </div>
              )}
            </div>
