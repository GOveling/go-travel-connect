...
export const getAIHotelBookingPlan = (trip: Trip): AIHotelBookingPlan => {
  ...
  destinationRanges.forEach((range, index) => {
    ...
    if (range.days === 1) {
      ...
      aiOptimized = true;
      reason =
        "IA extendió estancia mínima a 1 noche para reserva hotelera válida";
      optimizationNotes.push(
        `${destination.name}: Extendido de estancia de día a 1 noche`
      );
      ...
      optimizationNotes.push(
        `${destination.name}: Considerar tarifas de estancia larga`
      );
    } else {
  ...
  if (totalDestinations > 5) {
    confidence = "medium";
    optimizationNotes.push(
      "Itinerario complejo con múltiples destinos - revisar fechas"
    );
  }
  ...
  if (totalDestinations === 1) {
    confidence = "high";
    optimizationNotes.push(
      "Itinerario simple - fechas optimizadas automáticamente"
    );
  }
...
export const formatHotelBookingSummary = (plan: AIHotelBookingPlan): string => {
  const totalNights = plan.recommendations.reduce(
    (sum, rec) => sum + rec.nights,
    0
  );
...
export const getTotalHotelBudgetEstimate = (
