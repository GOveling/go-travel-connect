      ...
      <div className="relative">
        ...
        <Select value={selectedTripId} onValueChange={onTripSelection}>
          <SelectTrigger className="pl-10">
            <SelectValue placeholder="Elegir viaje para transportes automáticos" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="manual">
              <div className="flex flex-col">
                <span className="font-medium">Búsqueda manual</span>
                <span className="text-xs text-gray-500">Buscar transportes manualmente</span>
              </div>
            </SelectItem>
            {trips.map((trip) => (
            ...
            <div className="space-y-2">
              ...
              {selectedTrip.coordinates && selectedTrip.coordinates.length > 0 && (
                <div className="mt-3 p-2 bg-white rounded border border-blue-200">
                  <p className="text-xs font-medium text-blue-900 mb-1">🧠 IA analizará:</p>
                  <ul className="text-xs text-blue-800 space-y-1">
                    {selectedTrip.coordinates.map((dest, index) => (
                      <li key={index}>• Transportes para {dest.name}</li>
                    ))}
                  </ul>
                  <p className="text-xs text-blue-600 mt-2 font-medium">
                    ✨ Opciones automáticas: taxis, trenes, vuelos, car rental
                  </p>
                </div>
              )}
            </div>
