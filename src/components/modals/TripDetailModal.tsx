...
import { Calendar, MapPin, Users, Globe, Phone, Edit3, Share2, UserPlus, X, Plane, Car, Building, Clock, ExternalLink, Star, Heart, Navigation } from "lucide-react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import SavedPlacesRouteMap from "@/components/maps/SavedPlacesRouteMap";
...
const TripDetailModal = ({ trip, isOpen, onClose }: TripDetailModalProps) => {
  ...
  const [selectedDestinationForRoute, setSelectedDestinationForRoute] = useState<string | null>(null);
  ...
  const handleShowRoute = (destinationName: string, places: SavedPlace[]) => {
    if (places.length === 0) return;
    setSelectedDestinationForRoute(destinationName);
  ...
  return (
    <>
                      ...
                      <div className="flex items-center justify-between border-b pb-2">
                        ...
                        {placesForDestination.length > 0 && (
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => handleShowRoute(destination.name, placesForDestination)}
      ...
      {/* Route Map Modal */}
      {showRouteMap && selectedDestinationForRoute && (
        <SavedPlacesRouteMap
          destination={selectedDestinationForRoute}
          places={savedPlacesByDestination[selectedDestinationForRoute as keyof typeof savedPlacesByDestination] || []}
          onClose={() => {
            setShowRouteMap(false);
            setSelectedDestinationForRoute(null);
          }}
        />
      )}
    </>
