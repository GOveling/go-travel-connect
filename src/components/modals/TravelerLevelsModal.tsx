...
const TravelerLevelsModal = ({
  isOpen,
  onClose,
  currentLevel,
  totalPoints,
}: TravelerLevelsModalProps) => {
  return (
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="space-y-4">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="space-y-1">
                              ...
                              >
                                {isMaxLevel
                                  ? "Maximum Level"
                                  : `${level.requiredXP.toLocaleString()} XP required`}
                              </p>

                              {/* Unlocked Features Preview */}
                              <div className="flex flex-wrap gap-1">
                                {level.unlockedFeatures
                                  .slice(0, 2)
                                  .map((feature, index) => (
                                    <Badge
                                      key={index}
                            ...
                            <div className="text-right">
                              {!isMaxLevel && (
                                ...
                                >
                                  {isUnlocked
                                    ? "✓ Unlocked"
                                    : `${(level.requiredXP - totalPoints).toLocaleString()} XP to go`}
                                </p>
                              )}
                            </div>
                          </div>
          ...
          <div className="text-center text-sm text-gray-500 border-t pt-4">
            <p>
              Earn XP by visiting places, completing trips, and engaging with
              the community
            </p>
          </div>
        </div>
