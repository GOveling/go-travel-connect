...
const ManualFlightSummary = ({ flightType, flightData, multiCityFlights }: ManualFlightSummaryProps) => {
  ...
  if (flightType === 'multi-city') {
    return (
      <div className="space-y-4">
        <h3 className="font-medium text-gray-900">Resumen de Vuelos Multi-destino</h3>
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2 text-sm text-gray-600">
                  ...
                  <span>{format(new Date(flight.departDate), "dd/MM/yyyy")}</span>
                </div>
                
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center space-x-2 text-gray-600">
                    <Users size={14} className="text-gray-400" />
                    <span>{flight.passengers} pasajero{flight.passengers > 1 ? 's' : ''}</span>
                  </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <div className="flex flex-col">
                  <span>Salida: {format(new Date(flightData.departDate), "dd/MM/yyyy")}</span>
                  {flightType === 'round-trip' && flightData.returnDate && (
                    <span>Regreso: {format(new Date(flightData.returnDate), "dd/MM/yyyy")}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center space-x-2 text-gray-600">
                <Users size={14} className="text-gray-400" />
                <span>{flightData.passengers} pasajero{flightData.passengers > 1 ? 's' : ''}</span>
              </div>
              <Badge variant="secondary" className="text-xs">
