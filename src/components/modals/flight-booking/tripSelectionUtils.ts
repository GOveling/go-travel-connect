...
export const processTripSelection = (
  ...
  const optimizedDepartDate = adjustFlightDateBasedOnAI(
    tripStartDate,
    aiRecommendation
  );
...
const processMultiCityTrip = ({
  ...
  const daysPerDestination = Math.max(
    2,
    Math.floor(totalDays / destinations.length)
  );
  ...
  for (let i = 1; i < destinations.length; i++) {
    ...
    intermediateDate.setDate(
      intermediateDate.getDate() + i * daysPerDestination
    );
