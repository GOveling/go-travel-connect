...
export const getAIFlightTimingRecommendation = (
  ...
  if (distance < 500) {
    ...
    shouldDepartDayBefore = false;
    reason = 'Distancia media (500-1500km). Vuelo matutino el mismo día es óptimo.';
    confidence = 'high';
    ...
    if (timeZoneDifference > 3) {
      ...
      reason = 'Vuelo continental largo (1500-3000km) con diferencia horaria significativa. Día anterior recomendado.';
    } else {
      shouldDepartDayBefore = false;
      reason = 'Vuelo continental (1500-3000km). Vuelo temprano el mismo día es viable.';
    }
    confidence = 'high';
    ...
    shouldDepartDayBefore = true;
    reason = 'Vuelo largo (3000-6000km). Día anterior evita jet lag y permite descanso antes del itinerario.';
    confidence = 'high';
    ...
    shouldDepartDayBefore = true;
    reason = 'Vuelo muy largo (6000-10000km). Día anterior esencial para adaptación horaria y descanso.';
    confidence = 'high';
    ...
    shouldDepartDayBefore = true;
    reason = 'Vuelo extremadamente largo (>10000km). Día anterior + día de adaptación recomendado para jet lag severo.';
    confidence = 'high';
...
export const getOptimalMultiCityFlightTiming = (
  ...
  const flights: { from: string; to: string; departDate: string; aiOptimized: boolean }[] = [];
  ...
  for (let i = 0; i < locations.length - 1; i++) {
    ...
    const recommendation = getAIFlightTimingRecommendation(from, to, segmentDate.toISOString().split('T')[0]);
    const optimizedDate = adjustFlightDateBasedOnAI(segmentDate.toISOString().split('T')[0], recommendation);
