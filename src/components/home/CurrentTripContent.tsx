    ...
    if (nearestUpcomingTrip && !travelingTrip) {
      const calculateCountdown = () => {
        try {
          ...
          const year = nearestUpcomingTrip.dates.split(', ')[1] || new Date().getFullYear().toString();
      ...
      <Card className="overflow-hidden border-0 shadow-lg">
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">Traveling</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2 mb-3">
            <div className="bg-gradient-to-r from-green-600 to-blue-500 h-2 rounded-full w-3/7"></div>
          </div>
          <p className="text-sm text-gray-700 mb-3">Next: Optimized destination in route</p>
          <Button 
    ...
    return (
      <Card className="overflow-hidden border-0 shadow-lg">
        <div className="bg-gradient-to-r from-purple-600 to-orange-500 p-4 text-white">
          ...
          <p className="text-sm opacity-90">{nearestUpcomingTrip.destination}</p>
        </div>
        <CardContent className="p-4">
          <div className="text-center mb-4">
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="bg-gray-50 rounded-lg p-2">
                <div className="text-lg font-bold text-purple-600">{countdown.days}</div>
                <div className="text-xs text-gray-600">Days</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-2">
                <div className="text-lg font-bold text-purple-600">{countdown.hours}</div>
                <div className="text-xs text-gray-600">Hours</div>
              </div>
              <div className="bg-gray-50 rounded-lg p-2">
                <div className="text-lg font-bold text-purple-600">{countdown.minutes}</div>
    ...
    <Card className="overflow-hidden border-0 shadow-lg">
      ...
      <CardContent className="p-4">
        <div className="text-center mb-4">
          ...
          <p className="text-sm text-gray-600 mb-2">Ready for your next journey?</p>
          <p className="text-xs text-gray-500">Create a trip plan and let AI optimize your route</p>
        </div>
        <Button 
