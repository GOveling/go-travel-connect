...
export const useMapData = (trips: any[]) => {
  ...
  const filteredTrips = useMemo(() => {
    return trips.filter(trip => {
      ...
      // Group trip filter
      if (filters.isGroupTrip !== null && trip.isGroupTrip !== filters.isGroupTrip) {
        return false;
      }
      ...
      if (filters.dateRange !== 'all') {
        ...
        if (filters.dateRange === 'upcoming') {
          ...
          if (trip.status !== 'upcoming' && trip.status !== 'planning') return false;
  ...
  const allCoordinates = useMemo(() => {
    const coords: Array<{ lat: number; lng: number; trip: any; location: any; type: 'destination' | 'savedPlace' }> = [];
  ...
  const mapCenter = useMemo((): [number, number] => {
    if (allCoordinates.length === 0) return [40.7128, -74.0060]; // Default to NYC
  ...
  // Statistics
  const stats = useMemo(() => {
    const totalSavedPlaces = filteredTrips.reduce((total, trip) => 
      total + (trip.savedPlaces?.length || 0), 0
    );
