    ...
    const fetchUserAchievements = async () => {
      ...
      try {
        ...
        const { data: achievementsData, error: achievementsError } = await supabase
          .rpc('get_user_achievements_with_progress', { p_user_id: user.id });

        if (achievementsError) throw achievementsError;

        // Transform data to match our types
        const transformedAchievements: AchievementBadge[] = achievementsData?.map((item: any) => ({
          ...
          points: item.points,
          earnedDate: item.completed_at ? new Date(item.completed_at).toLocaleDateString('en-US', { 
            month: 'short', 
            day: 'numeric', 
            year: 'numeric' 
          }) : undefined,
