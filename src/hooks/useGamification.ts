...
import { TravelerLevel, AchievementBadge, GamificationProgress } from '@/types/gamification';
import { TRAVELER_LEVELS, ACHIEVEMENT_BADGES } from '@/data/gamificationData';

export const useGamification = () => {
  const [currentLevel, setCurrentLevel] = useState<TravelerLevel>(TRAVELER_LEVELS[0]);
  const [achievements, setAchievements] = useState<AchievementBadge[]>(ACHIEVEMENT_BADGES);
  ...
  useEffect(() => {
    const earnedAchievements = achievements.map(achievement => {
      ...
      if (currentLevel.level >= 2 && achievement.id === 'first-country') {
        return { ...achievement, earned: true, progress: 1, earnedDate: 'Dec 15, 2024' };
      }
      if (currentLevel.level >= 3 && achievement.id === 'city-explorer') {
        return { ...achievement, earned: true, progress: 5, earnedDate: 'Nov 20, 2024' };
      }
      if (currentLevel.level >= 4 && achievement.id === 'foodie') {
        return { ...achievement, earned: true, progress: 20, earnedDate: 'Oct 30, 2024' };
      }
      return achievement;
    });
