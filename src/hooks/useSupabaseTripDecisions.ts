...
export function useSupabaseTripDecisions(tripId: string) {
  ...
  const fetchDecisions = async () => {
    ...
    try {
      ...
      const decisionsWithVotes = decisionsData?.map(decision => ({
        ...decision,
        options: Array.isArray(decision.options) ? decision.options as string[] : [],
        selected_participants: Array.isArray(decision.selected_participants) ? decision.selected_participants as string[] : [],
  ...
  const createDecision = async (decisionData: Omit<TripDecision, "id" | "created_at" | "updated_at" | "created_by" | "votes">) => {
    if (!user) return;
    ...
    try {
      ...
      const transformedData = {
        ...
        selected_participants: Array.isArray(data.selected_participants) ? data.selected_participants as string[] : [],
  ...
  const updateDecision = async (id: string, decisionData: Partial<TripDecision>) => {
    try {
      const { data, error } = await supabase
        .from("trip_decisions")
        .update(decisionData)
      ...
      setDecisions(prev => prev.map(decision => 
        decision.id === id ? { ...transformedData, votes: decision.votes } : decision
      ));
    ...
    try {
      ...
      setDecisions(prev => prev.map(decision => {
        if (decision.id === decisionId) {
          const updatedVotes = decision.votes?.filter(v => v.user_id !== user.id) || [];
