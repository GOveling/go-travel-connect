...
import {
  ...
  createTentativeDestinationItinerary,
...
export const generateOptimizedRoutes = (trip: Trip | null) => {
  ...
  const routes = {
    current: [] as DayItinerary[],
    speed: [] as DayItinerary[],
    leisure: [] as DayItinerary[],
  };
  const destinationRanges = getDestinationDateRanges(
    trip.dates,
  ...
  trip.coordinates.forEach((destination, destIndex) => {
    ...
    if (hasSavedPlaces) {
      ...
      const currentSortedPlaces = savedPlaces.sort((a, b) => {
        const priorityOrder = { high: 3, medium: 2, low: 1 };
        return priorityOrder[b.priority] - priorityOrder[a.priority];
      });

      const currentDayGroups = distributePlacesAcrossDays(
        currentSortedPlaces,
      ...
      currentDayGroups.forEach((dayPlaces, dayIndex) => {
        if (dayPlaces.length > 0) {
          routes.current.push({
            ...
            places: convertToOptimizedPlaces(
              dayPlaces,
              "current",
              destination,
              1
      ...
      // Speed Route
      const speedSortedPlaces = savedPlaces.sort((a, b) => {
        const priorityOrder = { high: 3, medium: 2, low: 1 };
        return priorityOrder[b.priority] - priorityOrder[a.priority];
      });

      const speedPlacesPerDay = Math.min(
        4,
        Math.ceil(savedPlaces.length / destinationRange.days)
      );
      const speedDayGroups = distributePlacesAcrossDays(
        speedSortedPlaces.slice(0, speedPlacesPerDay * destinationRange.days),
        destinationRange.days,
        "speed"
      );
      ...
      // Leisure Route
      const leisureSortedPlaces = savedPlaces.sort((a, b) => {
        const priorityOrder = { high: 3, medium: 2, low: 1 };
        return priorityOrder[b.priority] - priorityOrder[a.priority];
      });

      const leisurePlacesPerDay = Math.max(
        1,
        Math.floor(savedPlaces.length / destinationRange.days)
      );
      const leisureDayGroups = distributePlacesAcrossDays(
        leisureSortedPlaces.slice(
          0,
          leisurePlacesPerDay * destinationRange.days
        ),
        destinationRange.days,
        "leisure"
      );
...
export const getRouteConfigurations = (
  trip: Trip | null
): { [key: string]: RouteConfiguration } => {
  const routes = generateOptimizedRoutes(trip);
  const destinationRanges = trip
    ? getDestinationDateRanges(trip.dates, trip.coordinates.length)
    : [];
  const totalDays = destinationRanges.reduce(
    (total, range) => total + range.days,
    0
  );
