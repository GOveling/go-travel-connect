...
export const getDestinationDateRanges = (
  tripDates: string,
  totalDestinations: number
) => {
  const destinationRanges: Array<{
    startDate: Date;
    endDate: Date;
    days: number;
    dateString: string;
  ...
  try {
    ...
    for (let i = 0; i < totalDestinations; i++) {
      const daysForThisDestination =
        baseDaysPerDestination + (i < extraDays ? 1 : 0);
...
export const getIndividualDayDates = (destinationRange: {
  startDate: Date;
  endDate: Date;
  days: number;
  dateString: string;
...
export const extractDestinationDateFromItinerary = (
  tripDates: string,
...
export const calculateDestinationDays = (
  tripDates: string,
  totalDestinations: number,
  trip: any
) => {
...
export const getDestinationDates = (
  ...
  try {
    ...
    const startDateStr = dateRange[0];
    const year =
      tripDates.split(", ")[1] || new Date().getFullYear().toString();
